...
class ConcatFusion(nn.Module):
    ...
    def forward(self, x, y):
        ...
        if self.args.pme:
            ...
            if self.training:
                output = mu_dul +  std_dul
                output_m = mu_dul
            else:
                output = mu_dul
                output_m = mu_dul
            # output = self.pooling(output).squeeze(dim=(2,3))
            # output_m = self.pooling(output_m).squeeze(dim=(2,3))

            out = self.fc_out(output)

            out_m = self.fc_out(output_m)
            out_s = self.fc_out(std_dul)
